...
namespace libf {
    ...
    public:
         ...
         */
        DataPoint() : D(0), data(0), owns_data(true) {}

        /**
         * Creates a new data point of size D that is not initialized
         */
        DataPoint(int D) : D(D), data(new float[D]), owns_data(true) {}
        ...
        /**
         * Creates a new data point from an existing array of doubles.
         */
        DataPoint(float* d, int size, bool free_data);
    ...
    public:
        ...
        explicit DataStorage(int layer_count) : classcount_multi(std::vector<int>(layer_count,0)) {}
        ...
        int & getClassLabel(int i )
        {
            return classLabels[i];
                ...
                /**
         * Returns the i-th class label for a specific layer.
         */
        int getClassLabelsMulti(int i , int layer) const
        {
            return classLabels_multi[i][layer];
        }

        /**
         * Returns the i-th class label for a specific layer.
         */
        int & getClassLabelsMulti(int i , int layer)
        {
            return classLabels_multi[i][layer];
        }

        /**
         * Returns the i-th class labels.
         */
        std::vector<int>  getClassLabelsMulti(int i) const
        {
            return classLabels_multi[i];
        }
        ...
        void addDataPointMulti(DataPoint* point, std::vector<int> labels, bool free = true)
        {
            dataPoints.push_back(point);
            classLabels_multi.push_back(labels);
            freeFlags.push_back(free);
            for(unsigned int i=0; i < labels.size(); i++){
                if (labels[i] >= classcount_multi[i])
                {
                    classcount_multi[i] = labels[i]+1;
                }
            }
        }
        ...
        int getClasscount_multi(int layer) const
        {
            return classcount_multi[layer];
        }

        int getMultiLayerCount() const
        {
            return classcount_multi.size();
        }

        std::vector<float> getInvertedClassFrequency() const{
            std::vector<float> freq(getClasscount(), 0);
            for (int n = 0; n < getSize(); n++)
            {
                freq[getClassLabel(n)]++;
            }
            for (int c = 0; c < getClasscount(); c++)
            {
                freq[c] = getSize()/freq[c];
            }
            return freq;
        }

        std::vector<float> getInvertedClassFrequency(int class_layer) const{
            std::vector<float> freq(getClasscount_multi(class_layer), 0);
            for (int n = 0; n < getSize(); n++)
            {
                freq[getClassLabelsMulti(n)[class_layer]]++;
            }
            for (int c = 0; c < getClasscount_multi(class_layer); c++)
            {
                freq[c] = getSize()/freq[c];
            }
            return freq;
        ...
        void bootstrapmulti(int N, DataStorage* dataStorage, std::vector<bool> & sampled) const;
    ...
    private:
         ...
         */
        int classcount;
        std::vector<int> classcount_multi;
        /**
        ...
        std::vector<std::vector<int> > classLabels_multi;
