...
DataPoint::DataPoint(float* d, int size, bool free_data)
{
  D = size;
  data = d;
  owns_data = free_data;
...
DataStorage::DataStorage(const DataStorage & other)
{
    ...
    classcount_multi = other.classcount_multi;
    classLabels_multi = other.classLabels_multi;
...
void DataStorage::bootstrapmulti(int N, DataStorage* dataStorage, std::vector<bool> & sampled) const
{
    // Set up a probability distribution
    std::random_device rd;
    std::mt19937 g(rd());
    ...
    for (int i = 0; i < N; i++)
    {
        // Select some point
        const int point = distribution(g);
        dataStorage->addDataPointMulti(getDataPoint(point), getClassLabelsMulti(point), false);
        sampled[point] = true;
    }
    dataStorage->classLabelMap = classLabelMap;
}
